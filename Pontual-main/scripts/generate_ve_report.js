...
const RECORDS_FILE = path.join('C:', 'Users', 'JD', 'Documents', 'Pontual', 'Relatorios', 'Records_AllDepts_260331_to_260430_5567.xls');
...
async function main() {
    console.log('Gerando relatório VE (Correção de Data e IDs)...');
    ...
    while ((match = cellRegex.exec(content)) !== null) {
        ...
        // Robust ID check: "1-Name", "1 - Name", etc.
    ...
    console.log(`Encontrados ${sortedIds.length} colaboradores.`);
...
<div class="no-print">
  <h2 style="margin:0 0 10px;color:#0369a1;">Relatório de Assiduidade — VE Vontade e Empenho</h2>
    ...
    for (const id of sortedIds) {
        ...
        ALL_DATES.forEach(d => {
            ...
            if (punches.length >= 2) {
                ...
                // VE Rule: OT starts at 6th minute (5 min tolerance subtracted)
                if (dur >= 486) {
                    const extra = dur - 485;
                    totalOtMin += extra;
        ...
        html += `<div class="page">
  <h1>Pontual | VE Vontade e Empenho</h1>
  ...
  <div class="sigs"><div class="sig">Assinatura do Colaborador</div><div class="sig">Assinatura do Responsável</div></div></div>`;
    ...
    html += '</body></html>';
    fs.writeFileSync(path.join(OUTPUT_DIR, 'Relatorio_VE_Final.html'), html, 'utf8');
    fs.writeFileSync(path.join(OUTPUT_DIR, 'Relatorio_VE_Final.csv'), csv, 'utf8');
    console.log('Relatórios VE gerados com sucesso!');
}
