$recordsFile = "C:\Users\JD\Downloads\Records_AllDepts_260831_to_260907_7502.xls"
$outputDir = "C:\Users\JD\Documents\Pontual\Relatorios"

Write-Host "A processar Records para Gengibre (31/08/2026 a 07/09/2026)..."

if (-not (Test-Path $recordsFile)) {
    Write-Error "Ficheiro nao encontrado: $recordsFile"
    exit 1
}

# Special chars via hex codes (garante 100% integridade em qualquer encoding)
$ch_a_ac = [char]0xE1  # á
$ch_e_ac = [char]0xE9  # é
$ch_i_ac = [char]0xED  # í
$ch_o_ac = [char]0xF3  # ó
...
foreach ($m in $cellMatches) {
    ...
    if ($val -match '^(\d+)\s*-\s*(.*)$') {
        ...
        $name = ($empM.Groups[2].Value.Trim() -replace '\s+', ' ')
    ...
    # Gengibre 7502: MM/DD/YYYY
    if ($val -match '^(\d{1,2})/(\d{1,2})/(\d{4})$') {
        if ($currentEmp) {
            $dm = [regex]::Match($val, '^(\d{1,2})/(\d{1,2})/(\d{4})$')
            $month = $dm.Groups[1].Value.PadLeft(2,'0')
            $day = $dm.Groups[2].Value.PadLeft(2,'0')
            $year = $dm.Groups[3].Value
...
$css = "body { font-family: 'Segoe UI', sans-serif; font-size: 11px; margin: 0; padding: 20px; color: #1e293b; background: #f8fafc; } " +
       ...
       ".exempt-box { margin-top: 10px; padding: 10px; background: #fef3c7; border-left: 4px solid #f59e0b; font-size: 10px; color: #92400e; } " +
...
$startDate = Get-Date "2026-08-31"
$endDate = Get-Date "2026-09-07"

$csv  = "sep=;`r`n"
$csv += "Relat${ch_o_ac}rio de Assiduidade - Gengibre`r`n"
$csv += "Per${ch_i_ac}odo: 31/08/2026 a 07/09/2026`r`n`r`n"
...
foreach ($id in $sortedIds) {
    ...
    $csv += "Colaborador: $($emp.name) ($id)`r`n"
    $csv += "Data;Entrada;Almo${ch_cced}o;Sa${ch_i_ac}da;Total;Extra;Obs`r`n"
    ...
    while ($curr -le $endDate) {
        ...
        $csv      += "$($curr.ToString('dd/MM/yyyy'));$e1;$almoco;$s2;$duration;$ot;$obsCSV`r`n"
    ...
    $safeName = [System.Net.WebUtility]::HtmlEncode($emp.name)

    $html += "<div class='page'><div class='header'><div class='header-info'><h1>Pontual | Gengibre</h1><p>Relat&oacute;rio de Assiduidade</p></div></div>"
    $html += "<div class='emp-box'><div><strong>Colaborador</strong><span>$safeName</span></div><div><strong>ID</strong><span>$id</span></div><div><strong>Per&iacute;odo</strong><span>31/08/2026 a 07/09/2026</span></div></div>"
    ...
    $csv += "TOTAL DO PER${ch_I_ac}ODO;;;;$(Fmt-Hms $totalWorkMin);$(Fmt-Hms $totalOtMin);`r`n"

    # Isencao de horario para Ademir (11) e Evelyn (18)
    if ($id -eq "11" -or $id -eq "18") {
        $payableOtMin = [Math]::Max(0, $totalOtMin - 1200)
        $html += "<div class='exempt-box'>"
        ...
        $csv += "Isen${ch_cced}${ch_a_ti}o de Hor${ch_a_ac}rio: as primeiras 20h est${ch_a_ti}o inclu${ch_i_ac}das no vencimento base.`r`n"
        if ($totalOtMin -le 1200) {
            $html += "Dentro da isen&ccedil;&atilde;o. Horas extra a pagar: 0h00m."
            $csv  += "Dentro da isen${ch_cced}${ch_a_ti}o. Horas extra a pagar: 0h00m.`r`n"
        } else {
            $html += "Excedeu a isen&ccedil;&atilde;o. <strong>Horas extra a pagar: $(Fmt-Hms $payableOtMin)</strong>"
            $csv  += "Excedeu a isen${ch_cced}${ch_a_ti}o. Horas extra a pagar: $(Fmt-Hms $payableOtMin)`r`n"
        }
        $html += "</div>"
    }
...
$html += "</body></html>"
$htmlPath = Join-Path $outputDir "Relatorio_Gengibre_31_Agosto_a_07_Setembro.html"
[System.IO.File]::WriteAllText($htmlPath, $html, [System.Text.Encoding]::UTF8)

$csvPath = Join-Path $outputDir "Relatorio_Gengibre_31_Agosto_a_07_Setembro.csv"
$utf16 = [System.Text.Encoding]::Unicode
[System.IO.File]::WriteAllText($csvPath, $csv, $utf16)

Write-Host "HTML e CSV gerados:"
Write-Host "   -> $htmlPath"
Write-Host "   -> $csvPath"

# Edge Headless PDF
$pdfPath = Join-Path $outputDir "Relatorio_Gengibre_31_Agosto_a_07_Setembro.pdf"
$edgeExe = "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
if (Test-Path $edgeExe) {
    Write-Host "A gerar PDF via Microsoft Edge..."
    & $edgeExe --headless --disable-gpu --run-all-compositor-stages-before-draw --no-pdf-header-footer --print-to-pdf="$pdfPath" "$htmlPath"
    if (Test-Path $pdfPath) {
        Write-Host "   -> PDF gerado com sucesso: $pdfPath"
    }
}
# Run node to create XLSX
$nodeScript = "C:\Users\JD\Documents\Pontual_Repo\scripts\create_gengibre_xlsx.js"
