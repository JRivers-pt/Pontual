...
$currentDataFile = "C:\Users\JD\Downloads\CurrentData_AllDepts_260601_to_260630_5567.xls"
$outputDir = "C:\Users\JD\Documents\Pontual\Relatorios"

# Define the months we want to generate reports for
$reportPeriods = @(
    @{
        MonthName = "Maio"
        StartDate = Get-Date "2026-05-01"
        EndDate = Get-Date "2026-05-31"
        RecordsFile = "C:\Users\JD\Documents\Pontual\Relatorios\Records_AllDepts_260501_to_260531_5567.xls"
        PeriodStr = "01/05/2026 a 31/05/2026"
    },
    @{
        MonthName = "Junho"
        StartDate = Get-Date "2026-06-01"
        EndDate = Get-Date "2026-06-30"
        RecordsFile = "C:\Users\JD\Downloads\Records_AllDepts_260601_to_260630_5567.xls"
        PeriodStr = "01/06/2026 a 30/06/2026"
    }
)
...
foreach ($period in $reportPeriods) {
    ...
    Write-Host "A processar o mês de $monthName com o ficheiro $recordsFile..."
    
    if (-not (Test-Path $recordsFile)) {
        Write-Warning "Ficheiro de registos não encontrado para $monthName em $recordsFile. A saltar..."
        continue
    }
    ...
    Write-Host "✅ Relatórios de $monthName gerados com sucesso!"
    Write-Host "   -> $htmlPath"
    Write-Host "   -> $csvPath"
}
